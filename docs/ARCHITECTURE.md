...
### Высокопроизводительное кэширование (Caffeine L1 Cache)
Для трансляции UUID в BIGINT используется двухфазная система поиска:
1. **L1 (In-Memory):** Библиотека **Caffeine** с алгоритмом вытеснения **W-TinyLFU**. В отличие от обычного LRU, этот алгоритм защищает систему от «вымывания» кэша при потоковом сканировании данных, сохраняя только наиболее часто используемые ключи.
2. **L2 (Database):** Таблица `migration_mapping`. Используется только при промахе L1-кэша (Cache Miss).

**Преимущество:** Гарантированное предотвращение утечек памяти (диагонального роста Heap) за счет жесткого лимита `maximumSize`. Пространственная сложность маппинга зафиксирована на уровне $O(1)$.
