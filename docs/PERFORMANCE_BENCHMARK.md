...
- `cacheLimit` и `maxPoolSize` фиксируются для сравниваемой серии. Размер batch основной миграции сейчас фиксирован в `DataMigrator` на 1000 строк.
...
.\gradlew.bat run --args="copy --mapping-strategy=EAGER --cache-limit 100000"
...
docker compose up -d
.\gradlew.bat run --args="generate-data --count 1000000 --truncate true --seed 42"
.\gradlew.bat run --args="copy --mapping-strategy=LAZY --cache-limit 100000"
New-Item -ItemType Directory -Force performance_runs | Out-Null
$latestRun = Get-ChildItem performance_logs -Directory | Sort-Object LastWriteTime -Descending | Select-Object -First 1
...
docker compose up -d
.\gradlew.bat run --args="generate-data --count 1000000 --truncate true --seed 42"
.\gradlew.bat run --args="copy --mapping-strategy=HYBRID --cache-limit 100000"
New-Item -ItemType Directory -Force performance_runs | Out-Null
$latestRun = Get-ChildItem performance_logs -Directory | Sort-Object LastWriteTime -Descending | Select-Object -First 1
...
| Метрика | Источник |
|---------|----------|
| strategy, cacheLimit, effective batch | `run_config.txt`, `batch_performance.csv` |
